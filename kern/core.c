...
static int dune_is_user_mode(void) { return 0; }
...
static long dune_dev_ioctl(struct file *filp, unsigned int ioctl,
  ...
  switch (ioctl) {
  case DUNE_ENTER:
    r = copy_from_user(&conf, (int __user *)arg, sizeof(struct dune_config));
    if (r) {
      r = -EIO;
      goto out;
    }
  ...
  case DUNE_GET_LAYOUT:
    ...
    r = copy_to_user((void __user *)arg, &layout, sizeof(struct dune_layout));
    if (r) {
      r = -EIO;
      goto out;
    }
  ...
  case DUNE_TRAP_DISABLE:
    ...
    break;
  case DUNE_REGISTER_COMMAND_CHANNEL:
    r = dune_register_cmd_channel(arg);
    break;
